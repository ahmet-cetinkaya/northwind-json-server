...
const login = (req, res, next) => {
  ...
  const userRoles = db.userRoles.filter((userRole) => userRole.userId === user.id);

  if (!user || user.password !== password) return next(new CustomError("Please check your credentials", 401));
